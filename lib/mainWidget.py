...
class MyMainForm(QMainWindow, Ui_MainWindow):
    ...
    def login(self):
        ...
        Logger().getLog().debug(" globalName.device_url %s" % globalName.device_url)
        if url:
            ...
            Logger().getLog().debug("【token：" +  globalName.device_token + "accountID：" + globalName.device_accountID + "】")
            # 获取设备列表
            ...
            if len(self.device):
                ...
                print("【" + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + "】： " + "设备为空")
            # 保存配置文件参数,需要读取配置，故此文件需要手动编辑，自动写入账号密码意义不大。--20220125 Eleven
            # self.cfg_save()
        else:
            print("【" + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + "】： " + "环境选择错误")
        con = configparser.ConfigParser()
        con.read(setting.TEST_CONFIG, encoding='utf-8')
        sectionName = con.sections()
        Logger().getLog().debug(sectionName)
    ...
    def running(self):
        if self.pushButton.text() == "开始测试":
            ...
            Logger().getLog().debug("全局变量testDevice--->%s" % globalName.testDevice)
    ...
    def Prepare_the_environment(self, device_url, data1):
        print("【" + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + "】：执行预知条件....")
        ...
        try:
            while True:
                ...
                deviceinfo["traceId"] = globalName.device_traceID
    ...
    def get_device_result(self, device_url, data1):
        print("【" + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + "】：设备状态查询中....")
        self.device_url = globalName.device_url
        self.data1 = data1
        device_traceID = int(round(time.time() * 1000))
        try:
            while True:
                ...
                data1 = json.loads(data1)
                print("结果——读取", data1)
                data1["traceId"] = globalName.device_traceID
