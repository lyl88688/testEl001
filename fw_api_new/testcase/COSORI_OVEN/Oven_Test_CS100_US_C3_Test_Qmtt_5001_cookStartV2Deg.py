...
# comDataStr = '{"context".*?"hasPreheat":0}}'
comDataStr = '{"context":{"traceId":"[0-9]*","method":"cookStartV2".*?"hasPreheat":.?}}'
...
class cosoriTest(unittest.TestCase):
    ...
    def testqmttgdeg(self):
        threads = []
        t1 = threading.Thread(target=cosoriTest().comData)
        threads.append(t1)
        t2 = threading.Thread(target=cosoriTest().cookstartQmtt, kwargs={"cookTemp":exceptTestDegval, "cookTime":exceptTestTime, "unit":exceptDegUnit})
        threads.append(t2)
        ...
        try:
            print(testComData)
            if testComData["data"]["cookStartTime"]:
                ...
                self.assertEqual(testComData["data"]["workTemp"], exceptTestDegval)
                self.assertEqual(testComData["data"]["mode"], exceptMode)
                self.assertEqual(testComData["data"]["level"], exceptlLevel)
                self.assertEqual(testComData["data"]["workTempUnit"], exceptDegUnit)
